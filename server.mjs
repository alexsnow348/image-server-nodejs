...
const IMAGE_DIR = isWindows
  ? "C:/images"
  : "/data/biolab_arralyze_picture_data/";
const INTERESTED_FOLDER = [
  "20240529 K562-NK cells purified_Killing Assay (9b33)",
  "20241219_Hek293 trypsination timelapse (3826)",
];
const IMAGES = [];
...
app.get("/images/list", async (req, res) => {
  try {
    ...
    const imageFiles = IMAGES.filter((image) =>
      path.relative(IMAGE_DIR, image).includes(experimentName)
